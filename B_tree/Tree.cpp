...
void Tree::getNodeToF(const Node* node, FILE*file) const
{
    if (node != nullptr)
    {
        getNodeToF(node->m_left, file);

        fprintf(file, "%s", node->m_english);
        fprintf(file, "%c", '-');
        fprintf(file, "%s", node->m_russian);
        fprintf(file, "%c", '\n');

        getNodeToF(node->m_right, file);
    }
}
